...
async function publishActivity(req, res) {
  ...
  const sql =
    "INSERT INTO `volunteer_activities` (`activity_name`, `activity_content`, `activity_location`, `number_of_recuits`, `contact_name`, `contact_phone`, `start_time`, `end_time`) VALUES (?,?,?,?,?,?,?,?)";
  const parmas = [activity_name, activity_content, activity_location, number_of_recuits, contact_name, contact_phone, start_time, end_time];
  try {
