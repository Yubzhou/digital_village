...
router.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    const result = await executeSql("SELECT * FROM `refresh_tokens_volunteers` WHERE `volunteer_id`=? AND `iat`=? LIMIT 1", [decoded.sub, decoded.iat]);
    ...
    const now = Math.floor(Date.now() / 1000); // 使用UTC当前时间戳（默认单位为秒，因为jsonwebtoken包默认使用UTC且单位为秒）
    const payload = { sub: decoded.sub, isAdmin: decoded.isAdmin, iat: now, exp: now + jwtConfig.ACCESS_TOKEN_EXPIRATION };
    ...
    // 保存RefreshToken到数据库
    await executeSql("UPDATE `refresh_tokens_volunteers` SET `iat`=?, `exp`=? WHERE `volunteer_id`=?", [now, payload.exp, decoded.sub]);
...
router.post("/logout", async (req, res) => {
  ...
  try {
    ...
    await executeSql("UPDATE `refresh_tokens_volunteers` SET `iat`=0 WHERE `volunteer_id`=? LIMIT 1", [volunteerID]);
